...
# APPENDIX: Find number of claims with less than or equal to 90 bene's served (lvl 1 and nt only)
print('Appendix: ',final_matched_claims_allyears[(final_matched_claims_allyears['num_bene_served']<=90)&(final_matched_claims_allyears['TRAUMA_LEVEL'].isin(['1']))].shape[0]+final_unmatched_claims_allyears[final_unmatched_claims_allyears['num_bene_served']<=90].shape[0])

# Create indicator if more than 90
final_matched_claims_allyears['less_than_90_bene_served'] = np.where((final_matched_claims_allyears['num_bene_served']<=90)&(final_matched_claims_allyears['TRAUMA_LEVEL'].isin(['1','2','3'])),1,0)
final_unmatched_claims_allyears['less_than_90_bene_served'] = np.where((final_unmatched_claims_allyears['num_bene_served']<=90),1,0)
...
final_matched_claims_allyears_w_hos_qual.to_stata('/mnt/labshares/sanghavi-lab/Jessy/data/trauma_center_project_all_hos_claims_for_reg/merged_ats_claims_for_stata/final_matched_claims_allyears_w_hos_qual.dta',write_index=False)
final_unmatched_claims_allyears_w_hos_qual.to_stata('/mnt/labshares/sanghavi-lab/Jessy/data/trauma_center_project_all_hos_claims_for_reg/merged_ats_claims_for_stata/final_unmatched_claims_allyears_w_hos_qual.dta',write_index=False)
