...
for b in mile_binary:
    ...
    sns.barplot(x="Area type", y="Adjusted 30 day mortality (%)", hue = "hos_amb_type", data = concat_df[concat_df['Radius']=='3 miles'],
                ax=ax1,errorbar=None,width=0.8,palette=palette_colors,zorder=0)
    sns.barplot(x="Area type", y="Adjusted 30 day mortality (%)", hue = "hos_amb_type", data = concat_df[concat_df['Radius']=='5 miles'],
                ax=ax2,errorbar=None,width=0.8,palette=palette_colors,zorder=0)

    palette_colors = {"NT_BLS": "pink", "lvl1_BLS": "pink", "NT_ALS": "pink", "lvl1_ALS": "pink"}
    sns.barplot(x="Area type", y="Adjusted 30 day mortality (%)", hue = "hos_amb_type", data = concat_df[concat_df['Radius']=='3 miles'],
                ax=ax1,errorbar=None,width=0.8,palette=palette_colors,zorder=1)
    sns.barplot(x="Area type", y="Adjusted 30 day mortality (%)", hue = "hos_amb_type", data = concat_df[concat_df['Radius']=='5 miles'],
                ax=ax2,errorbar=None,width=0.8,palette=palette_colors,zorder=1)
    ...
    for i, thisbar in enumerate(ax1.patches):
        ...
        if i in [*range(0,8)]:
            ...
            if i in [8,9]: # Define positions of bar
                color = "navy"
            elif i in [10,11]: # Define positions of bar
                color = "darkgoldenrod"
            elif i in [12,13]: # Define positions of bar
                color = "darkgreen"
            elif i in [14,15]: # Define positions of bar
                color = "maroon"
            ...
            # Set specifications for bar modifications to decrease size of bar and change position
            current_width = thisbar.get_width()
            diff = current_width - (-0.10)

            # Change the bar width
            thisbar.set_width(-0.10)
    ...
    ax1.set_title("Radius = 3 miles")
    ax2.set_title("Radius = 5 miles")

    #position of second graph
    ax2.set_position([0.50, 0.11, 0.37, 0.78])
