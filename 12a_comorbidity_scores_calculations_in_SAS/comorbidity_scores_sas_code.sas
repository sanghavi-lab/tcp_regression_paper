...
/* Running the following script in terminal to run in phs: "/usr/local/SASHome/SASFoundation/9.4/sas" -SYSIN /mnt/labshares/sanghavi-lab/Jessy/hpc_utils/codes/python/trauma_center_project/final_regression_analysis_paper_two/12a_comorbidity_scores_calculations_in_SAS/comorbidity_scores_sas_code.sas -LOG /mnt/labshares/sanghavi-lab/Jessy/hpc_utils/codes/sas/trauma_center_project/log/job1.log */

/* Need following code to prevent "ERROR: Insufficient authorization to access" when executing in linux */
ods listing close;

/* Read in data (CSV) */
proc import datafile="/mnt/labshares/sanghavi-lab/Jessy/data/trauma_center_project_all_hos_claims/comorbidity_score/2017_for_sas.csv"
	out=diag_imp replace;
guessingrows=2000000; /* This will "guess" the column type through examining 7k rows. Need this to force DX column as character. */
delimiter=',';
datarow=2;
getnames=YES;
run;
...
proc export 
  ...
  dbms=xlsx 
  outfile="/mnt/labshares/sanghavi-lab/Jessy/data/trauma_center_project_all_hos_claims/comorbidity_score/files_output_from_sas/2017_sas_output.xlsx"
  replace;
run;
